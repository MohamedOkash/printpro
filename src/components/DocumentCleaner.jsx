...
export default function DocumentCleaner() {
  ...
  return (
    <div className="h-full flex flex-col md:flex-row md:items-stretch gap-4 md:p-3 relative pb-14 md:pb-0">
      {/* Toolbar (Mobile top) */}
      ...
      <AnimatePresence>
        {activeSection && activeSection !== 'upload' && (
          <motion.div
            ...
            className="fixed inset-0 z-40 flex items-end md:items-center md:justify-end md:pr-8 lg:pr-16"
            onClick={() => setActiveSection('')}
          >
            <motion.div
              className="w-full md:w-[42%] bg-[rgba(12,14,20,0.72)] border border-white/6 backdrop-blur-lg rounded-t-[18px] md:rounded-l-[18px] md:rounded-tr-none p-4 shadow-glow"
                  ...
                  <div className="space-y-3">
                    <div className="flex gap-2 overflow-x-auto">
                      {FILTER_PRESETS.map(p => (
                        <button key={p.id} onClick={() => store.applyPreset(p.id)} className="min-w-[84px] py-2 bg-white/5 rounded-lg">{p.label}</button>
                      ))}
                    </div>
